...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
     ...
     **/
    int mazeSize = horiz * vert;
    DisjointSets cells = new DisjointSets(mazeSize);
    int horWalls = horiz * (vert - 1);
    int vertWalls = (horiz - 1) * vert; 
    int totalWalls = horWalls + vertWalls;
    int[] allNum = new int[totalWalls];
    ...
    {
    	randNum = randInt(totalWalls - i);
    	int offset = 0;
    	while(allNum[offset] == -1)
    	{
    		offset++;
    	}
    	for(j = 0;j < randNum;j++)
    	{
    		if(allNum[j+offset] == -1)
    		{
    			j--;
    			offset++;
    		}
    	}
    	while(allNum[j+offset] == -1)
    	{
    		offset++;
    	}
    	int actRand = allNum[j+offset];
    	allNum[j+offset] = -1;
    	if(actRand < horWalls)
		{
			int nthRow = actRand%(vert - 1);
			int nthCol = actRand/(vert - 1);
			int cell1 = nthCol * vert + nthRow;
			int cell2 = cell1 + 1;
			int set1 = cells.find(cell1);
			int set2 = cells.find(cell2);
			if(set1 != set2)
			{
				hWalls[nthCol][nthRow] = false;
				cells.union(set1, set2);
			}
		}
		else
		{
			actRand = actRand - horWalls;
			int nthRow = actRand%vert;
			int nthCol = actRand/vert;
			int cell1 = nthCol * vert + nthRow;
			int cell2 = cell1 + vert;
			int set1 = cells.find(cell1);
			int set2 = cells.find(cell2);
			if(set1 != set2)
			{
				vWalls[nthCol][nthRow] = false;
				cells.union(set1, set2);
			}
		}
